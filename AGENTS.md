...
src/exit-status.js             — FAILURE_EXIT_CODE, operationalFailureCount(result), applyExitCode(result, setExitCode) — single source of truth for the failure→exit-code contract; consumed by withErrorHandling
...
- **Boundary validation for external JSON** — all untrusted JSON is validated at the read boundary via `src/validate-json.js` before reaching core logic. `validateConfig` is called inside `loadConfig` (the file-not-found path still returns `null`; a shape error propagates as `ValidationError`). `validateSenders` / `validateClassifications` are called in `classify-command.js` and `import-classifications-command.js` (single-item commands — throw on bad shape). `validateImportEntries` is called in `import-classifications-command.js`. `validateSidecar` is called inside the `walkOutputTree` visitors in `receipt-output-tree.js` (batch traversal — caught by the surrounding try/catch and routed to `onError`+continue, exactly like a JSON parse failure). `ValidationError` carries `code: "INVALID_INPUT"` so `withErrorHandling` emits a machine-readable `--json` error.
- **Canonical error escalation patterns for command orchestrators:**
  ...
  - *Per-item degradation* (`processReceiptMessage`'s caller, `walkOutputTree`, `pdfToText`, `processDownloadMessage`, `searchMailboxForReceiptRecords`) — long-running batch traversals catch **operational** failures per item, emit a progress event, tally into `stats.errors` / `failures[]`, and continue. They **must** call `rethrowIfProgrammerError()` (from `src/programmer-error.js`) first so `TypeError`/`ReferenceError`/`RangeError`/`SyntaxError` escape to the command boundary. `searchMailboxForReceiptRecords` (`src/receipts/receipt-mailbox-search.js`) owns this call for both `scanForReceipts` and `searchMailboxForReceipts`, which delegate to it as thin adapters.
- **Exit-code escalation contract** — every CLI action registered via `wrapAction` returns its command orchestrator's result (or `undefined`). `withErrorHandling` (`src/cli-helpers.js`) passes that result through `applyExitCode()` (`src/exit-status.js`), which reads `operationalFailureCount()` — the sum of any non-zero `result.stats.{errors,failed,timedOut,searchFailures,indexErrors,accountFailures}` fields plus `result.accountFailures.length` — and sets `process.exitCode = 1` when the count is non-zero. This means a failed operation is never indistinguishable from a successful empty result: `forEachAccount()` (`src/imap-client.js`) returns `{ accountFailures }` instead of swallowing connect errors, batch commands (`move`, `flag`) fold connect failures into `stats.failed` alongside their existing item-level `status: "failed"` entries, and `receipts extract`'s three modes (download/reprocess/listVendors) all surface their failure counts under `result.stats` so none of them can print a `⚠ … failed` warning while still exiting 0. Read-only commands (`search`, `inbox`, `contacts`, `folders`) surface partial-outage results via `result.accountFailures`, print a `⚠` warning (`accountFailuresWarning()` in `src/format-mail.js`) instead of silently returning an empty result, and include `accountFailures` in `--json` output when non-empty.
