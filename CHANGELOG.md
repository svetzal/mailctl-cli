...
- **Updated dependencies to their latest versions:** `imapflow` 1.7.8 → 2.0.6, `nodemailer` 9.1.1 → 10.0.10, and `cmx-core` 0.3.0 → 0.4.0. The two major bumps are packaging/tooling changes, not API changes: both `imapflow` and `nodemailer` were rewritten in TypeScript, now ship ESM + CJS builds with their own generated type declarations, and now require Node.js 20+ (mailctl ships as a Bun-compiled binary, and both upstreams explicitly support Bun, so the engine floor has no effect here). Every IMAP and SMTP call mailctl makes is unchanged, including XOAUTH2 for Microsoft 365. The one visible effect is that `imapflow`'s generated types are more honest than its old hand-written ones: `client.search()` is now correctly typed as possibly returning `undefined` (it always could, when no mailbox is selected), so `ImapGateway.search()`'s JSDoc was widened to match. All call sites already guarded for a falsy result, so there is no behavior change.
- **Raised the `ip-address` override floor from `^10.7.0` to `^10.7.2`.** The override exists as a security floor (it was originally added to pin past the `ip-address` SSRF advisories) and is pulled in transitively by `socks` via `imapflow`. The pin held resolution at 10.7.0 even though newer patches were published; the floor now tracks the latest patch. No direct dependency of mailctl changed, and `bun audit` reports no known vulnerabilities.
