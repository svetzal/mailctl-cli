...
describe("applyReceiptFilters", () => {
  it("returns all results unchanged when no vendor filter and no exclusions", () => {
    ...
    const { filtered, vendorExcluded, subjectExcluded } = applyReceiptFilters(
      results, {}, matchesVendor, NO_EXCLUSIONS
    );
  ...
  it("filters messages not matching the vendor", () => {
    ...
    const { filtered, vendorExcluded } = applyReceiptFilters(
      results, { vendor: "acme" }, matchesVendor, NO_EXCLUSIONS
    );
  ...
  it("excludes messages matching subject exclusion patterns", () => {
    ...
    const { filtered, subjectExcluded } = applyReceiptFilters(
      results, {}, matchesVendor, SOME_EXCLUSIONS
    );
    expect(filtered).toHaveLength(1);
  ...
  it("applies vendor filter before subject exclusions", () => {
    const results = [
      ...
      makeMsg({ uid: 2, messageId: "msg2@example.com", fromAddress: "noreply@github.com", fromName: "GitHub", subject: "Payment date approaching" }),
      makeMsg({ uid: 3, messageId: "msg3@example.com", fromAddress: "billing@acme.com", fromName: "Acme", subject: "Payment date approaching" }),
    ];
    const { filtered, vendorExcluded, subjectExcluded } = applyReceiptFilters(
      results, { vendor: "acme" }, matchesVendor, SOME_EXCLUSIONS
    );
  ...
  it("does not apply vendor filter when opts.vendor is null", () => {
    const results = [makeMsg({ fromAddress: "billing@acme.com" })];
    const { filtered, vendorExcluded } = applyReceiptFilters(
      results, { vendor: null }, matchesVendor, NO_EXCLUSIONS
    );
    expect(filtered).toHaveLength(1);
    expect(vendorExcluded).toBe(0);
  });
