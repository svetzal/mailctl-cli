...
/**
 * Download, validate, and write a single PDF part, recording its outcome in the
...
async function downloadAndRecordPdfPart(client, msg, part, vendor, context) {
  const { manifestKey, manifest, dryRun, outputDir, existingFiles, existingHashes, fs, onProgress } = context;
...
export async function processDownloadMessage(client, msg, mailbox, context) {
  ...
  for (const part of pdfParts) {
    const partAction = await downloadAndRecordPdfPart(client, msg, part, vendor, partContext);
    if (partAction !== null) action = partAction;
  }
